...
function platon_preprocess_page(&$vars) {
 ...
 //最左边的主菜单：home, course_categories, forum, message, and etc.
  $vars['main_navigation'] = _platon_get_main_navigation();

    // Create the OG context tabs.
   // course, class, school, institute 右上方的工具Tab：home of this og, edit, memebers, tools -- video, files, live meeting, and etc.
  if (module_exists('og_context')) {
    ...
    if (!empty($group['gid'])) {
      ...
      foreach (array(
        ...
        //milton: intro, refer to audio, post. advertise: members post
        "node/{$group['gid']}/intro" => array(
          'class' => 'platon-og-context-view-tab platon-og-context-users-tab',
        ),
      //milton: video, files, quiz, live meeting, forum, and etc.
      ...
      $tabs[] = array(  //milton: 以menu_local_task来显示
      ...
      //Miton: Add tools with icons in the top menu
      //
      if ((isset($vars['node']))&&(og_is_group('node', $vars['node']))) {
        $groups = $vars['node'];
        //milton: 获得og的tools -- opigno_course_tools
  ...
  //Milton: new from 1.23 ------------- need review
  if (arg(0) == 'node') {
  ...
  //milton: 左边栏，在webex（zoom）、lesson中，override _preprocess_page,将所在group下面的所有对应的node set给  $vars ['group_state']
...
/**
 * render list of group: lesson, webex, in house training, live-meetings，等主要的项目
 * modified by milton to suit for class
 */
function platon_render_group_state($var)
{
    $empty = true;
    $html = '<div id="opigno-group-progress">';
    //milton: begin --- new from 1.23
    //milton: certificate & process & scores
...
function platon_preprocess_node(&$vars) {
  ...
  if (defined('OPIGNO_COURSE_BUNDLE') && !empty($vars['node']->type) && in_array($vars['node']->type, array(OPIGNO_COURSE_BUNDLE, 'class'))) {
    ...
    //milton: add 2 arraies of fields' name: one for middle (under body), one for the right
    //$vars['content']['fields'][1]['body']


    $middle_arr = array(
        'school' =>  array('field_overview_arr','field_student_arr','field_faculty_arr','field_sports_arr','field_extracurriculars_arr','field_admission_arr')
    );

    $right_arr = array(
        'class' => array('opigno_class_expiration','class_quota','opigno_commerce_price','field_start_time'),
        'course' => array('opigno_commerce_price','opigno_user_expiration_amount','opigno_user_expiration_type','course_required_quiz_ref','course_required_course_ref','course_pretest_ref','field_books'),

        'school' => array('field_amount','field_amount_teacher','field_school_type','field_address','field_grades','field_coed',' field_association','field_days','field_hours','field_library','field_website','field_phone','field_fax','field_email','field_deadline','field_per_color','field_per_international','field_avg_classsize','field_amt_ap','field_tuition','field_year','field_religious','field_dresscode'),

        'institute' => array('field_address','field_website','field_year','field_phone','field_email','field_fax','field_services')
    );
    if(isset($middle_arr[$vars['node']->type])){
      $i = 2;
      foreach($middle_arr[$vars['node']->type] as $key ){
          $vars['content']['fields'][$i][$key]['#prefix'] = '<div class="body-field"><div class="label">'.$vars['content'][$key]['#title'] .'</div><div class="content">';
          $vars['content']['fields'][$i][$key] = $vars['content'][$key];

          $vars['content']['fields'][$i][$key]['#suffix'] = '</div></div>';
          $i++;
      }
    }
    if(isset($right_arr[$vars['node']->type])){
      $i = 1;
      foreach($right_arr[$vars['node']->type] as $key ){

          $vars['content']['group_group'][$i][$key] = $vars['content'][$key];
          //$vars['content']['group_group'][$i]['#title'] = $vars['content'][$key]['#title'];
          //$vars['content']['group_group'][$i][$key]['#suffix'] = '</div></div>';
          $i++;
      }
    }
    //print(debug($vars['content']['group_group']));
    //print_r($vars['content']['group_group']);
...
/* add by Jack, used for header menu --begin */
/* main ul */
function platon_menu_tree__menu_header_menu($variables) {
  return '<ul id="top-menu" class="nav navbar-nav navbar-right main-nav">' . $variables['tree'] . '</ul>';
}

/* inner ui */
function platon_menu_tree__menu_header_menu_inner($variables) {
    return '<ul class="dropdown-menu" role="menu">' . $variables['tree'] . '</ul>';
}

/* inner li */
function platon_menu_link__menu_header_menu_inner($variables) {
...
/* main li */
function platon_menu_link__menu_header_menu(array $variables) {
  $element = $variables['element'];
  $sub_menu = '';
  ...
  if ($element['#below']) {
    foreach ($element['#below'] as $key => $val) {
      if (is_numeric($key)) {
        $element['#below'][$key]['#theme'] = 'menu_link__menu_header_menu_inner'; // 2 lavel
      }
    }
    $element['#below']['#theme_wrappers'][0] = 'menu_tree__menu_header_menu_inner';  // 2 lavel
    $sub_menu = drupal_render($element['#below']);
  }
  $output = l($element['#title'], $element['#href'], $element['#localized_options']);
...
/* add by Jack, used for header menu --end */
